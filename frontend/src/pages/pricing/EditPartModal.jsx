...
const EditPartModal = ({ isOpen, onClose, onSuccess, partToEdit }) => {
  ...
  useEffect(() => {
    if (isOpen && partToEdit) {
      setPartName(partToEdit.partName || '');
      setError(null);
      setSuccessMsg('');
    }
  }, [isOpen, partToEdit]);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      // partToEdit should contain id mapping according to earlier findings (part.id)
      const response = await PricingService.updatePart(partToEdit.id, payload);
      
      setSuccessMsg(response.data?.message || 'Part updated successfully.');
