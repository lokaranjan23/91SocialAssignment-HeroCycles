...
const UpdatePartPriceModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const fetchParts = async () => {
    ...
    setError(null);
    try {
      const response = await PricingService.getPricingParts();
  ...
  const handleSave = async () => {
    ...
    // strip time from today for accurate date-only comparison
    ...
    try {
      const payload = {
        partPrices: priceUpdates,
        effectiveFrom: effectiveFrom
      };
      
      const response = await PricingService.updatePartPrices(payload);
      ...
      setError(err.response?.data?.message || 'Failed to update part prices.');
      setSaving(false);
    }
  };
  ...
  return (
    <div style={modalOverlayStyle}>
      <div style={modalContentStyle}>
        <h3 style={{ marginTop: 0 }}>Bulk Update Part Prices</h3>
        ...
        <div style={{ flex: 1, overflowY: 'auto', marginTop: '1rem' }}>
          {loadingData ? (
            ...
            <p>No parts available to update.</p>
          ) : (
                ...
                {parts.map(part => (
                  <tr key={part.id}>
                    ...
                    <td style={{ padding: '12px', border: '1px solid #ddd', color: '#666' }}>
                      {part.effectiveFrom || 'Pending Change'}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid #eee' }}>
          ...
          <button 
            type="button"
            onClick={handleSave}
            disabled={saving || loadingData || parts.length === 0}
...
export default UpdatePartPriceModal;
